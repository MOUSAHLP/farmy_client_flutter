...
class BasketBloc extends Bloc<BasketEvent, BasketState> {
  ...
  int countsProducts(int id) {
    if (mutableProducts.any((element) => element.id == id)) {
      int index =
      mutableProducts.indexWhere((element) => element.id == id);
  ...
  int productPrice(int id){
    ...
    return (mutableProducts[index].quantity! * int.parse(mutableProducts[index].price??''));
  }
  int finalPrice(){
    int sum=0;
    for(int i=0;i<mutableProducts.length;i++){
      sum+=(mutableProducts[i].quantity! * int.parse(mutableProducts[i].price??''));
    }
    return sum;
  }
